...
shortyGen :: R.Connection -> IO (Either R.Reply [Char])
shortyGen rConn = do
  shorty <- replicateM 7 (randomElement alphaNum)
  entry <- liftIO (getURI rConn $ BC.pack shorty)
  case entry of
    Left reply -> return $ Left reply
    Right mbBS -> case mbBS of
      Nothing -> return  $ Right shorty
      Just _ -> shortyGen rConn
...
app rConn = do
  get "/" $ do
    ...
    case parsedUri of
      Just _ -> do
        shawtyAttempt <- liftIO (shortyGen rConn)
        case shawtyAttempt of
